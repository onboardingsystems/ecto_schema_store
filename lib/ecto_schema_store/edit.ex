      ...
      * `errors_to_map`    - If an error occurs, the changeset error is converted to a JSON encoding firendly map. When given an atom, sets the root id to the atom. Default: `false`
      ...
      def continue(%EventQueues.Event{} = event) do
        ...
        response =
          case event.name do
            :before_insert ->
              execute_insert(event.data.previous, event.data.params, event.data.options)

            :before_update ->
              execute_update(event.data.previous, event.data.params, event.data.options)

            :before_delete ->
              execute_delete(event.data.previous)
        ...
        case repo.insert(change) do
          ...
          {:ok, model} = result ->
            ...
            if has_after_insert?() do
              event =
                EctoSchemaStore.Event.new(
                  current_action: :after_insert,
        ...
        if has_before_insert?() do
          ...
          if sync do
            ...
            after
              timeout ->
                {:error,
                 "Insert operation timed out (exceeded #{timeout} milliseconds) for #{
                   inspect(params)
                 }. This insert may still be completed."}
            end
          else
            :ok
          end
        ...
        if has_before_update?() do
          ...
          if sync do
            ...
            after
              timeout ->
                {:error,
                 "Update operation timed out (exceeded #{timeout} milliseconds) with #{
                   inspect(model)
                 } for #{inspect(params)}. This update may still be completed."}
            end
          else
            :ok
          end
        ...
        if has_before_delete?() do
          ...
          if sync do
            ...
            after
              timeout ->
                {:error,
                 "Delete operation timed out (exceeded #{timeout} milliseconds) for #{
                   inspect(model)
                 }. This delete may still be completed."}
            end
          else
            :ok
          end
